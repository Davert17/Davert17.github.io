...
document.getElementById('tablebut').onclick=XMLajaxcontent;
}
...
function XMLajaxcontent(){
    ...
    te.innerText="testword";
    //document.getElementById('ajaxsection').appendChild(te);
    var xhttp = new XMLHttpRequest();
    xhttp.open("GET","xmldom.xml",true);
    xhttp.send();
    if(xhttp.readyState==4)
    {
        var xmlreq=xhttp.responseXML;
        var newnode=xmlreq.getElementById('xmlcontent');
        document.getElementById('ajaxsection').appendChild(newnode);
    }
}
